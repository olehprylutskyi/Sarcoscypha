...
This repository contains the code, data, and extended methodology for the article "Distribution and ecology of genus *Sarcoscypha* in Ukraine: what can we learn from citizen science, morphology, barcoding, and eDNA?" by Yuliia Leshchenko, Oleh Prylutskyi, & Iryna Yatsiuk, published in *Czech Mycology* (2025). The article is available at [https://doi.org/10.33585/cmy.77206](https://doi.org/10.33585/cmy.77206).
...
Since both MODIS-derived and plant functional traits data had finer spatial resolution than CHELSA climeate data, we rescaled those rasters to the same spatial extent and resolution as CHELSA, using the *terra* R package v. 1.8 (Hijmans et al., 2025), by consecutivelly apply *terra::project(rester-to-be-rescaled, CHELSA-raster)* and *terra::mask(rester-to-be-rescaled, CHELSA-raster)*. Those procedures ensure all covariate rasters had the same spatial projection, resolution, and pixel boundaries.

All covariates were then cropped by the Ukraine administrative boundary and by a custom polygon mask of large inland water bodies using the *raster* R package v. 3.6 (Hijmans, 2025). The total number of environmental variables was 37.
